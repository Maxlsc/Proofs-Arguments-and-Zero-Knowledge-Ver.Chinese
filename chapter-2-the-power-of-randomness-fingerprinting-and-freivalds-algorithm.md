...
$$
\begin{aligned} \forall x\neq y,\mathop{\text{Pr}} \limits_{h\in\mathcal{H}}[h(x)=h(y)]\leq0.0001. \end{aligned}
$$
...
$$
\begin{align} \mathcal{H}=\{h_r:r\in\mathbb{F}_p\} \tag{2.1} \end{align}
$$
...
​ 第一个性质的正确性显而易见：如果$$a=b$$，那么显然对任意可能的$$r$$都有$$h_r(a)=h_r(b)$$。第二个性质的正确性基于下面的关键事实（Fact），我们将在[2.1.6](broken-reference)节中给出该事实的证明。

**Fact 2.1.** 对任意两不同（distinct、unequal）的最大维数为$$n$$的多项式$$p_a,p_b\in\mathbb{F}_p^n$$，至多有$$n$$个不同的$$x\in\mathbb{F}_p$$使得$$p_a(x)=p_b(x)$$。
...
### 2.1.6 对Fact [2.1](broken-reference)的解释

Fact [2.1](broken-reference)事实上等同于下述事实：

**Fact 2.2.** 任意非零且维数至多为$$n$$的多项式在任意域上至多有$$n$$个根。

​ 对Fact [2.2](broken-reference)的一个简单的证明是[\[hp\]](https://math.stackexchange.com/users/677/dan)。为了说明Fact [2.2](broken-reference)等价于Fact [2.1](broken-reference)，注意到如果$$p_a$$和$$p_b$$是两个维数最多为$$n$$的不同的多项式，且最多只有$$n$$个$$x\in\mathbb{F}_p$$使得$$p_a(x)=p_b(x)$$，那么$$p_a-p_b$$就是非零且维数至多为$$n$$，但有多于$$n$$个根的多项式。
